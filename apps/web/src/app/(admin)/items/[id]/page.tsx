...
export default function EditItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/items/${id}`, {
        method: 'PUT',
        headers: {
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-[400px]">Loading item details...</div>;
  }
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500">Update item details and tax configuration.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-4">
          ...
          <Button type="submit" isLoading={saving} className="px-8">Update Item</Button>
        </div>
      </form>
    </div>
  );
